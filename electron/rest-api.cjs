// Synapse REST API — OpenAI-compatible gateway for all providers
...
// ─── Config ──────────────────────────────────────────────
const REST_PORT = parseInt(process.env.SYNAPSE_REST_PORT) || 3210;
const VERSION = '4.1.0';
const API_PREFIX = '/v1';
...
function formatAllResponse(allResults) {
    ...
    return {
        ...
        model: 'all',
        choices,
        synapse: { providers: allResults.models, timings: allResults.timings }
    };
}
...
function getChatJS() {
    ...
    ws.onerror=()=>{addSystem('Connection error — is Synapse running?');};}
...
<body>
    ...
    <div class="wrap">
        ...
        <div class="head">
            <div class="logo">⚡ Synapse API</div>
...
{"model": "auto", "message": "Hello"}</pre>
        ...
        <div class="foot">Synapse API v${VERSION} — Zen4-bit ⚡</div>
    </div>
    ${getChatJS()}
</body>
</html>`;
...
<body>
    ...
    <div class="wrap">
        ...
        <div class="head">
            <div class="logo">🖥️ Synapse CLI</div>
...
node cli/synapse-cli.cjs ask claude "What is AI?"
...
synapse ask claude "Hello"</pre>
        ...
        <div class="sec">
            ...
            <div class="ex">
                ...
                <pre>
# Specific provider
synapse ask claude "Explain quantum computing"
synapse ask chatgpt "Write a poem about AI"
synapse ask gemini "Summarize this topic"
synapse ask perplexity "Latest news on AI"

# Auto-pick best available
synapse ask "Hello"
synapse ask auto "Hello"

# All providers at once
synapse ask all "What is consciousness?"
synapse compare "Is water wet?"</pre>
            </div>
        </div>
...
# Python error → AI fix
python app.py 2>&1 | synapse fix

# Any command output
docker logs app | synapse ask "any errors?"</pre>
                </div>
                <div class="ex">
                    <h4>📄 File as Context</h4>
                    <pre># Explain a file
synapse ask "explain this" --file src/app.js

# Review a file
synapse ask "review for bugs" --file server.py

# Fix error with source file
synapse fix "TypeError" --file src/utils.js</pre>
                </div>
                <div class="ex">
                    <h4>🔀 Pipe + Question</h4>
                    <pre># Log analysis
cat error.log | synapse ask "what went wrong?"

# Git changes → code review
git diff | synapse code review

# Config check
cat nginx.conf | synapse ask "any issues?"</pre>
                </div>
                <div class="ex">
                    <h4>⚡ Auto-Fix Mode</h4>
                    <pre># Just pipe anything — auto detects
npm test 2>&1 | synapse fix
cargo build 2>&1 | synapse fix
go build . 2>&1 | synapse fix

# Even without a command name:
some-command 2>&1 | synapse</pre>
                </div>
            </div>
        </div>
        ...
        <div class="sec">
            ...
            <div class="ex">
                ...
                <pre>
# Generate code
synapse code "REST API with auth" --lang Python
synapse code "sort algorithm" --lang JavaScript

# Review code
synapse code review "def fib(n): return fib(n-1)+fib(n-2)"
cat app.js | synapse code review

# Explain code
synapse code explain "async/await patterns"
cat complex.py | synapse code explain

# Debug
synapse code debug "function fails on empty array"</pre>
            </div>
        </div>
...
--json         Raw JSON output</pre>
                ...
                <div class="ex">
                    ...
                    <pre># Custom port
set SYNAPSE_PORT=4000
synapse ask claude "Hello"

# Custom host
set SYNAPSE_HOST=192.168.1.100
synapse status
...
# Default: 127.0.0.1:${REST_PORT}</pre>
        ...
        <div class="foot">Synapse CLI v${VERSION} — Zen4-bit ⚡</div>
    </div>
    ${getChatJS()}
</body>
</html>`;
...
<body>
    ...
    <div class="wrap">
        ...
        <div class="head">
            <div class="logo">🔌 Synapse WebSocket</div>
...
print(result["content"])</pre>
        ...
        <div class="foot">Synapse WebSocket v${VERSION} — Zen4-bit ⚡</div>
    </div>
    ${getChatJS()}
</body>
</html>`;
...
End with a security score (0-100).`;
        ...
        if (fn === 'debate') {
            ...
            sendJSON(res, 200, {
                id: `synapse-${Date.now()}`, object: 'chat.completion', model: 'debate',
                topic, perspectives: debateResults, timings: debateTimings,
                synapse: { function: 'debate', providers: resolved2.providers.slice(0, sides) }
            });
            return;
        }
    ...
    if (method === 'GET' && pathname === `${API_PREFIX}/models`) {
        ...
        const models = enabled.map(p => ({
            ...
            owned_by: 'synapse', status: 'enabled',
        ...
        allProviders.filter(p => !enabled.includes(p)).forEach(p => {
            models.push({
                id: p, object: 'model', owned_by: 'synapse', status: 'disabled',
                aliases: Object.entries(MODEL_ALIASES).filter(([_, v]) => v === p).map(([k]) => k).filter(k => k !== p)
            });
        });
        models.push({ id: 'auto', object: 'model', owned_by: 'synapse', description: 'Auto-picks best available model' });
    ...
    if (method === 'GET' && pathname === '/api/status') {
        ...
        sendJSON(res, 200, {
            success: true, server: 'Synapse API', version: VERSION,
...
function startRestAPI() {
    ...
    httpServer.listen(REST_PORT, '127.0.0.1', () => {
        ...
        console.log(`[API] ⚡ Synapse API v${VERSION} running at http://localhost:${REST_PORT}`);
