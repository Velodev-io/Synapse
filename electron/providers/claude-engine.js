 ...
 */
(function() {
    if (window.__synapseClaude) return;
    ...
    async function _createConversation(orgId, promptPreview) {
        const res = await fetch('/api/organizations/' + orgId + '/chat_conversations', {
            ...
            body: JSON.stringify({
                name: (promptPreview || 'synapse').substring(0, 50).replace(/\n/g, ' ').trim(),
                project_uuid: null,
                is_starred: false
            })
        });
    ...
    async function send(message) {
        ...
        if (!_convId) {
            ...
            console.log('[Synapse Claude] Created new conversation:', _convId);
        } else {
            console.log('[Synapse Claude] Continuing conversation:', _convId);
        }
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 404 || res.status === 410) {
                    console.log('[Synapse Claude] Conversation expired, creating new one...');
                    _convId = await _createConversation(orgId, message);
    ...
    function newConversation() {
        _convId = null;
        console.log('[Synapse Claude] Conversation reset');
    }

    window.__synapseClaude = { send: send, newConversation: newConversation };
    console.log('[Synapse] Claude engine loaded');
})();
