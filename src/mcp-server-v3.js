#!/usr/bin/env node
// Synapse MCP Server v4.1.0 — IPC bridge to Electron Agent Hub
...
function getEnabledProviders() {
    try {
        ...
        if (process.platform === 'win32') {
            appDataPath = path.join(process.env.APPDATA || '', 'synapse', 'enabled-providers.json');
        } else if (process.platform === 'darwin') {
            appDataPath = path.join(process.env.HOME || '', 'Library', 'Application Support', 'synapse', 'enabled-providers.json');
        } else {
            appDataPath = path.join(process.env.HOME || '', '.config', 'synapse', 'enabled-providers.json');
        }
    ...
    return new Set(['perplexity', 'chatgpt', 'gemini', 'claude', 'deepseek', 'kimi']);
}
...
function getFileReferenceEnabled() {
    try {
        ...
        let settingsPath;
        if (process.platform === 'win32') {
            settingsPath = path.join(process.env.APPDATA || '', 'synapse', 'settings.json');
        } else if (process.platform === 'darwin') {
            settingsPath = path.join(process.env.HOME || '', 'Library', 'Application Support', 'synapse', 'settings.json');
        } else {
            settingsPath = path.join(process.env.HOME || '', '.config', 'synapse', 'settings.json');
        }
...
class AIProvider {
    ...
    async ensureInitialized() {
        ...
        if (!isProviderEnabled(this.name)) {
            throw new Error(`${this.name} is disabled. Enable it in Synapse Agent Hub settings.`);
        }
...
const router = new SmartRouter({ perplexity, chatgpt, claude, gemini, deepseek, kimi });
...
async function attachRAGContext(message, limit = 5) {
    try {
        console.error('[MCP RAG] Automatically searching codebase for query context...');
        const result = await ipcClient.send('workspaceRagSearch', null, { query: message, limit });
        if (result.success && Array.isArray(result.result) && result.result.length > 0) {
            let context = '\n\n--- Semantic Code Context (Workspace RAG) ---\n';
            result.result.forEach(chunk => {
                context += `\nFile: ${chunk.file_path} (Lines ${chunk.start_line}-${chunk.end_line})\n\`\`\`\n${chunk.content}\n\`\`\`\n`;
            });
            console.error(`[MCP RAG] Attached ${result.result.length} semantic code snippets.`);
            return `${message}${context}`;
        }
    } catch (e) {
        console.error('[MCP RAG] Failed to retrieve context:', e.message);
    }
    return message;
}
...
server.tool(
    'search_workspace',
    {
        query: z.string().describe('Keyword or concept to search for in the workspace codebase'),
        limit: z.number().optional().describe('Maximum number of search results to return (default: 5)')
    },
    async ({ query, limit }) => {
        try {
            const result = await ipcClient.send('workspaceRagSearch', null, { query, limit: limit || 5 });
            if (result.success && Array.isArray(result.result)) {
                if (result.result.length === 0) {
                    return toolResponse("No matching code snippets found in the workspace.");
                }
                
                let output = `# Search Results for "${query}"\n\n`;
                result.result.forEach((chunk, index) => {
                    output += `### Result ${index + 1}: [${chunk.file_path}](file:///${chunk.file_path}) (Lines ${chunk.start_line}-${chunk.end_line})\n`;
                    output += `\`\`\`\n${chunk.content}\n\`\`\`\n\n`;
                });
                return toolResponse(output);
            } else {
                return toolResponse(`Search failed: ${result.error || 'Unknown error'}`);
            }
        } catch (err) {
            return toolError(err);
        }
...
server.tool(
    'workspace_rag_chat',
    {
        message: z.string().describe('The user query or instruction'),
        provider: z.string().optional().describe('AI provider to use: chatgpt, claude, gemini, perplexity, deepseek, kimi. Default: auto-select'),
        limit: z.number().optional().describe('Number of semantic code chunks to retrieve (default: 5)')
    },
    async ({ message, provider: providerName, limit }) => {
        try {
            const p = providerName ? resolveProvider(providerName) : pickBestProvider('coding');
            if (!p) return toolResponse('No providers available.');
            
            // Automatically search workspace and attach context
            const semanticMessage = await attachRAGContext(message, limit || 5);
            console.error(`[MCP RAG] Sending query to ${p.name} with attached codebase context...`);
            const response = await p.instance.chat(semanticMessage);
            return toolResponse(response);
        } catch (err) {
            return toolError(err);
        }
...
function pickBestProvider(taskType) {
    const enabled = getEnabledProviders();
    const providers = { perplexity, chatgpt, claude, gemini, deepseek, kimi };
    
    // Priority order based on task type
    const priorities = {
        coding: ['claude', 'chatgpt', 'deepseek', 'gemini', 'kimi', 'perplexity'],
        research: ['perplexity', 'deepseek', 'gemini', 'chatgpt', 'claude', 'kimi'],
        general: ['claude', 'chatgpt', 'deepseek', 'gemini', 'kimi', 'perplexity'],
        review: ['claude', 'chatgpt', 'deepseek', 'gemini', 'kimi', 'perplexity']
    };
...
server.tool(
    'ask_selected',
    {
        ...
        providers: z.array(z.string()).describe('Which providers to ask: ["chatgpt", "claude", "gemini", "perplexity", "deepseek", "kimi"]'),
    ...
    async ({ message, providers: selectedProviders, files }) => {
        try {
            const enabled = getEnabledProviders();
            const allProviders = { perplexity, chatgpt, claude, gemini, deepseek, kimi };
