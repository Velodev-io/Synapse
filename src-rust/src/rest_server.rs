...
fn format_chat_response(text: &str, provider: &str, response_time_ms: u64) -> Value {
    json!({
        "id": format!("proxima-{}", chrono::Utc::now().timestamp_millis()),
        ...
        "proxima": { "provider": provider, "responseTimeMs": response_time_ms }
    })
}
...
async fn handle_completions(
    ...
    if fn_field == "debate" {
        ...
        return Json(json!({
            ...
            "timings": timings,
            "proxima": { "function": "debate", "providers": resolved.iter().take(sides).collect::<Vec<_>>() }
        })).into_response();
    }
    ...
    if resolved.len() == 1 {
        ...
        Json(json!({
            ...
            "proxima": { "providers": resolved, "timings": timings }
        })).into_response()
    }
}
...
<body style="font-family: sans-serif; background: #0c0c16; color: #fff; padding: 40px;">
    <h1>⚡ Proxima Local REST API running (Rust sidecar)</h1>
