<script lang="ts">
  ...
  function exportAsJSON() {
    ...
    a.href = url;
    a.download = `synapse_history_${new Date().toISOString().slice(0, 10)}.json`;
  ...
  function exportAsMarkdown() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `synapse_history_${new Date().toISOString().slice(0, 10)}.md`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  onMount(async () => {
    ...
    if (!agentHub) {
      console.error('[Synapse] Electron preload API not found');
      return;
    }
    ...
    try {
      ...
      agentHub.onProviderNavigated((data: any) => {
        console.log('[Synapse UI] Provider navigated:', data);
      });
    } catch (e: any) {
      console.error('[Synapse UI] Init failed:', e);
    }
  });
  ...
  void function updateCliUI() {
    if (cliInstalled) {
      ...
      cliInstallStatus = '✅ CLI is active. Use "synapse" from any terminal.';
    } else {
  ...
  <main class="main-content">
    ...
    {#if activeTab === 'welcome'}
      <div class="welcome-panel">
        <h2 class="welcome-title">Welcome to Synapse</h2>
        ...
        <div class="welcome-steps">
          ...
          <div class="welcome-step">
            ...
            <div class="welcome-step-content">
              ...
              <div class="welcome-step-desc">Hook Synapse up to Cursor/VS Code and ask queries.</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <table class="analytics-table">
              ...
              <tbody>
                {#if Object.keys(statsData.providers).length === 0}
                  <tr>
                    <td colspan="6" class="table-empty">No calls recorded yet. Send queries using Synapse or CLI to see metrics.</td>
                  </tr>
                {:else}
