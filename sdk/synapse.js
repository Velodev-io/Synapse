// Synapse SDK - JavaScript/Node.js client
//
// Usage:
...
class SynapseResponse {
    constructor(data) {
        ...
        this.responseTimeMs = synapse.responseTimeMs || 0;
        this.provider = synapse.provider || this.model;
 ...
 */
    constructor({ baseUrl = null, apiKey = null, model = 'auto', timeout = 120000, maxRetries = 3 } = {}) {
        const port = typeof process !== 'undefined' && process.env && process.env.SYNAPSE_PORT ? process.env.SYNAPSE_PORT : '3210';
     ...
     *
     * @returns {SynapseResponse} with .text, .model, .responseTimeMs
     *
     * @example
     * // Chat
        ...
        for (let attempt = 0; attempt < this.maxRetries; attempt++) {
            try {
                ...
                if (err.name === 'AbortError') {
                    ...
                    lastError = new Error(
                        `Cannot connect to Synapse at ${this.baseUrl}. ` +
                        `Is the Synapse app running? (attempt ${attempt + 1}/${this.maxRetries})`
                    );
                } else {
    ...
    async _post(endpoint, body) {
        ...
        return new SynapseResponse(await res.json());
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { Synapse, SynapseResponse };
}
