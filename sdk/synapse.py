...
class SynapseResponse:
    """Standardized response from any Synapse API call."""
    ...
    def __init__(self, data: dict):
        ...
        self.response_time_ms = synapse.get("responseTimeMs", 0)
        self.provider = synapse.get("provider", self.model)
    ...
    def __repr__(self):
        return f"SynapseResponse(model='{self.model}', text='{self.text[:50]}...')"
...
class Synapse:
    """
    Synapse API Client — ONE function for everything.
    ...
    def __init__(self, base_url=None, api_key=None, default_model="auto"):
        import os
        port = os.environ.get("SYNAPSE_PORT", "3210")
    ...
    def chat(self, message="", *, model=None, function=None, **kwargs):
        ...
        Returns:
            SynapseResponse with .text, .model, .response_time_ms
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = ConnectionError(
                    f"Cannot connect to Synapse at {self.base_url}. "
                    f"Is the Synapse app running? (attempt {attempt + 1}/{max_retries})"
                )
    ...
    def _post(self, endpoint, body):
        ...
        return SynapseResponse(resp.json())
